    ...
    if num_players >= 2:
        ...
        while running:
            ...
            if game.ball.y >= game.game_area_height * 0.75 and not voted:
                ...
                # Después de los 3 segundos, encontrar la posición ganadora
                max_votes = max(votes)  # Encontrar el máximo número de votos
                # Filtrar posiciones con el máximo número de votos (manejo de empate)
                possible_winners = [i for i, v in enumerate(votes) if v == max_votes]
                paddle_position_index = random.choice(possible_winners)  # Escoger una opción aleatoria en caso de empate
