...
from game import ArkanoidGame, start_multiplayer_game, draw_text_centered  # Importa la clase ArkanoidGame
import database
...
def mode_menu(window, window_width, game_area_height, font, small_font):
    ...
    while menu_running:
        ...
        draw_text_centered("SELECCIONA MODO DE JUEGO", font, WHITE, window, window_width // 2, game_area_height // 2 - 100)
        draw_text_centered("1. Modo Solitario", small_font, WHITE, window, window_width // 2, game_area_height // 2)
        draw_text_centered("2. Modo Multijugador", small_font, WHITE, window, window_width // 2, game_area_height // 2 + 50)
...
def main():
    ...
    while running:
        ...
        if mode == 'solitario':
            ...
            # Iniciar el flujo de modo multijugador (implementaremos esto a continuación)
            start_multiplayer_game(window, game, button_positions, font, small_font, connection, user_id)
        elif mode == 'exit':
            running = False
