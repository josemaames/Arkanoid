...
def start_multiplayer_game(window, game, button_positions, font, small_font, connection, user_id):
    ...
    # Esperar a que se conecten al menos 2 jugadores (en este caso, lo simularemos por ahora)
    connected_players = wait_for_players(window, font)
    ...
    if connected_players >= 2:
        ...
        while running:
            ...
            vote_result = collect_votes(window, button_positions, font, small_font)
...
def wait_for_players(window, font):
    """ Simula la espera de jugadores conectados """
    players = 0
    while players < 2:
        window.fill(BLACK)
        draw_text_centered(f"Esperando jugadores... Conectados: {players}/2", font, WHITE, window, WINDOW_WIDTH // 2, GAME_AREA_HEIGHT // 2)
        pygame.display.update()
        time.sleep(1)  # Simulación de tiempo de espera
        players += 1  # Simulación de que un jugador se conecta

    return players

def collect_votes(window, button_positions, font, small_font):
    """ Simula el proceso de votación entre los jugadores """
    votes = [0, 0, 0, 0]  # Conteo de votos para cada botón (Izq, Izq Med, Der Med, Der)

    # Simulamos la votación de los jugadores (en un juego real, esto se haría mediante red)
    for _ in range(2):  # Simulamos que hay 2 jugadores
        chosen_option = random.randint(0, 3)  # Simulación de una votación aleatoria
